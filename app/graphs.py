...
from data import distance_per_club, driving_accuracy_data, performance_by_par, gps_traces
...
def gap_analysis():
    ...
    fig = px.box(df, x='club', y='yards', category_orders={
...
def driving_accuracy(**kwargs):
    ...
    pivot_df = df.pivot_table(index='club', columns='fairwayshotoutcome', values='count', fill_value=0)
...
def map():
    ...
    fig = px.scatter_mapbox(df, lat="lat", lon="lon", color="scorecardid",
